...
QueueManager.prototype = {
    ...
    // callback is onMessage function given by ConnectionManager
    addMessageListener: function (exchange, queueName, routeName, callback) {
        for (var i in QueueManager.providers) {
            QueueManager.providers[i].createAndBind(exchange, queueName, routeName, function (msg) {
                callback(msg);
            });
        }
    },
    ...
    create: function (queueKey) {
        for (var i in QueueManager.providers) {
            QueueManager.providers[i].create(queueKey);
        }
    }
